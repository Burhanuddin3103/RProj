...
do_nxt.default <- function(e) {
  ## Using the stored list of "official" rproj variables and values,
...
do_reset.default <- function(e) {
  ...
  e$reset <- TRUE
  e$iptr <- 2
  rproj_out(s()%N%"I just reset the script to its original state. If it doesn't refresh immediately, you may need to click on it.", 
            skip_after = TRUE)
}
...
do_submit.default <- function(e) {
  ...
  rproj_out(s()%N%"Sourcing your script...", skip_after = TRUE)
  try(source(e$script_temp_path, encoding = "UTF-8"))
}

do_play.default <- function(e) {
  rproj_out(s()%N%"Entering play mode. Experiment as you please, then type nxt() when you are ready to resume the lesson.", skip_after=TRUE)
  e$playing <- TRUE
}

do_main.default <- function(e) {
  rproj_out(s()%N%"Returning to the main menu...")
...
do_restart.default <- function(e) {
  rproj_out(s()%N%"This feature is not implemented yet for rproj.")
}
