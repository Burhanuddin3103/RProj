...
# without the need to change core rproj source code. 
# 
...
runTest.newcmd <- function(keyphrase,e){
 ...
 #   identical(as.expression(e$expr)[[1]], as.expression(correct.expr)[[1]])
  if(ansResults$passed && callResults$passed){
    ...
    if (ansResults$passed && !callResults$passed){
      rproj_out("That's not the expression I expected but it works.")
      rproj_out(callResults$message)
      #todo
...
runTest.rproj1cmd <- function(keyphrase,e){
  correct.expr <- parse(text=rightside(keyphrase))
  correct.ans  <- eval(correct.expr)
...
runTest.in_range <- function(keyphrase, e){
  range <- try(eval(parse(text=paste0("c(", rightside(keyphrase), ")"))),
               silent=TRUE)
  if(!is.numeric(range)){
    rproj_out(paste("The given range", rightside(keyphrase), "is not numeric."))
    return(FALSE)
  }
  results <- expectThat(e$var, 
                        in_range(range, 
...
expectThat <- function(object, condition, info=NULL, label=NULL){
  ...
  results <- rprojExpectation(condition(object))
...
rprojExpectation <- function(testthat_expectation){
  ...
  structure(
    ...
    class = c("rproj_expectation", "expectation")
  )
}
