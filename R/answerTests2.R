#' Answer Tests
#'
#' Answer tests are how rproj determines whether a user has answered
...
#' 
#' @details
#' For each question that you author as part of a rproj lesson, you 
...
#' 
#' If you're using the \href{https://github.com/rprojdev/rprojify}{rprojify}
...
#' answer a command question, you must determine how you'd like rproj to
...
#'   in the answer tests. Since rproj sessions are continuous, accepting
...
#' are using the rprojify authoring tool, then a file called
...
omnitest <- function(correctExpr=NULL, correctVal=NULL, strict=FALSE, eval_for_class=as.logical(NA)){
  ...
  if(!is.null(correctVal)){
    if(is.character(e$val)){
      valResults <- expectThat(e$val,
                               ...
                               label=(e$val))
      if(is(e, "dev") && !valResults$passed)rproj_out(valResults$message)
  ...
  if((isTRUE(valGood) || is.na(valGood)) && exprGood){
    return(TRUE)
      ...
      rproj_out(s()%N%"That's not the expression I expected but it works.")
      rproj_out(s()%N%"I've executed the correct expression in case the result is needed in an upcoming question.")
...
expr_identical_to <- function(correct_expression){
  ...
  results <- expectThat(expr, 
                        ...
                        label=deparse(expr))
  if( is(e, "dev") && !results$passed)rproj_out(results$message) 
...
var_is_a <- function(class, var_name) {
  ...
  if(exists(var_name, globalenv())){
    ...
    if(is(e,"dev"))rproj_out(paste(var_name, "does not exist."))
    return(FALSE)
  }
}
