...
courseraCheck <- function(e){
  ...
  rproj_out(s()%N%"Are you currently enrolled in the Coursera course associated with this lesson?")
  yn <- select.list(c("Yes","No"), graphics=FALSE)
  if(yn=="No")return()
  ss <- lapply(1:2, function(i) {
    paste0(sample(tt, sample(seq(400), 1), replace=TRUE), collapse="")
  })
  rproj_out(s()%N%"Would you like me to notify Coursera that you've completed this lesson?",
  ...
  while(!ok) {
    ...
    if(choice=="Yes"){
      rproj_out(s()%N%"I'll try to tell Coursera you've completed this lesson now.")
      ...
      if(!is(ch, "try-error") && ch_ok) {
        ...
        if(!length(results)) {
          rproj_out(s()%N%"You skipped too many questions! You'll need to complete",
        ...
        if(!is(results, "try-error")){
          ...
          if(!str_detect(results, "[Ee]xception")){
            rproj_out(paste0(s()%N%"I've notified Coursera that you have completed ",
          ...
          rproj_out(s()%N%"I'm sorry, something went wrong with automatic submission.")
        ...
        rproj_out(s()%N%"I'm sorry, something went wrong with establishing connection.")
        # Exit loop if user doesn't want to retry auto submission
        ok <- !retry()
      }
    } # end of yes branch
  } # end of while loop
  rproj_out(s()%N%"To notify Coursera that you have completed this lesson,",
            ...
            s()%N%"button next to the appropriate rproj lesson.",
...
retry <- function() {
  rproj_out(s()%N%"Would you like to retry automatic submission or just submit manually?")
...
get_courseid <- function() {
  rproj_out(s()%N%"The first item I need is your Course ID. For example, if the",
  ...
  repeat {
    ...
    # courseid is one of the bad things
      ...
      if(is_url) {
        rproj_out(s()%N%"It looks like you entered a web address, which is not what I'm",
                  s()%N%"looking for.")
      }
      # Check if courseid is all numbers
      if(is_numbers) {
        rproj_out(s()%N%"It looks like you entered a numeric ID, which is not what I'm",
                  s()%N%"looking for.")
      }
      # Check if the user stole the example courseid
      if(is_example) {
        rproj_out(s()%N%"It looks like you entered the Course ID that I used as an",
                  s()%N%"example, which is not what I'm looking for.")
      }
    }
    rproj_out(s()%N%"Instead, I need your Course ID, which is the last",
...
getCreds <- function(e) {
  ...
  while(!confirmed) {
    ...
    rproj_out(s()%N%"Is the following information correct?", skip_after=TRUE)
