#' Installing Courses
#' 
#' rproj is designed so that anyone can create interactive content
...
#' from our official \href{https://github.com/rprojdev/rproj_courses}{course repository}. Otherwise, check out the
#' help file for the relevant install function below.
#' 
#' You can uninstall a course from rproj at any time with 
...
#' Install a course from The rproj Course Network or install a course from a
#' local .swc file.
#' 
#' @description 
#' Version 2.4 of rproj introduces a new, simple, and fast way of installing
#' courses in the form of \code{.swc} files. This function allows a user to grab
#' a \code{.swc} file from The rproj Course Network which is maintained by Team
#' rproj, or the user can use this function to install a local \code{.swc} file.
...
#' \dontrun{
#' 
#' # Install the latest version of Team rproj's R Programming course.
...
install_course <- function(course_name = NULL, swc_path = NULL, force = FALSE){
  ...
  if(!is.null(course_name) && !is.null(swc_path)){
    ...
    unpack_course(swc_path, rproj_courses_dir())
  } else { # install from rproj course network
    ...
    if(response$status_code != 200){
      rproj_out(s()%N%"It looks like your internet connection is not working.",
    ...
    unpack_course(temp_swc, rproj_courses_dir(), force = force)
  }
}
...
#' rproj courses at \url{https://github.com/rprojdev/rproj_courses}.
#' This function provides the easiest method of installing a course
#' form the repository.
#' 
#' We have another repository at 
#' \url{https://github.com/rprojdev/rproj_misc}, where we keep 
#' experimental features and content. The \code{dev} argument allows
#' you to access this repository. Content in the rproj_misc repository
#' is not guaranteed to work.
#' 
#' The central repository of rproj courses is mirrored at
#' \url{https://bitbucket.org/rprojdevmirror/rproj_courses}. If you cannot 
#' access GitHub you can download rproj courses from bitbucket by using the
#' \code{mirror = "bitbucket"} option (see below).
#' 
#' @param course_name The name of the course you wish to install.
#' @param dev Set to \code{TRUE} to install a course in development from the rproj_misc repository.
#' @param mirror Select rproj course repository mirror. Valid arguments are \code{"github"} and \code{"bitbucket"}.
#' @export
#' @importFrom httr GET content progress
#' @examples
#' \dontrun{
#' 
#' install_from_rproj("R_Programming") # Directory name
#' 
#' ### OR ###
#' 
#' install_from_rproj("R Programming") # Course name
#' 
#' # To install a course in development from the rproj_misc repository
#' install_from_rproj("Including Data", dev = TRUE)
#' 
#' # To install a course from the Bitbucket mirror
#' install_from_rproj("R Programming", mirror = "bitbucket")
#' }
#' @family InstallCourses
install_from_rproj <- function(course_name, dev = FALSE, mirror = "github"){
  # Validate arguments
  ...
  if(dev) {
    if(mirror != "github"){
      stop(s()%N%"To access rproj courses in development on Bitbucket go to https://bitbucket.org/rprojdevmirror/rproj_misc")
    }
  ...
  path <- file.path(rproj_courses_dir(), "temp.zip")
  ...
  # Extract
  unzip(path, exdir=rproj_courses_dir(), files=unzip_list)
  
  # Copy files from unzipped directory into Courses
  top_dir <- file.path(rproj_courses_dir(), sort(dirname(unzip_list))[1])
  dirs_to_copy <- list.files(top_dir, full.names=TRUE)
  if(file.copy(dirs_to_copy, rproj_courses_dir(), recursive=TRUE)){
    rproj_out(s()%N%"Course installed successfully!", skip_after=TRUE)
  } else {
    rproj_out(s()%N%"Course installation failed.", skip_after=TRUE)
  }
  ...
  unlink(file.path(rproj_courses_dir(), "__MACOSX"), recursive=TRUE, force=TRUE)
...
zip_course <- function(path, dest=NULL){
  ...
  if(file.copy(path, zip_dir, recursive=TRUE)){
    rproj_out("Course directory was successfully zipped!", skip_after=TRUE)
  } else {
    rproj_out("Course installation failed.", skip_after=TRUE)
  }
...
uninstall_course <- function(course_name){
  path <- file.path(rproj_courses_dir(), make_pathname(course_name))
  if(file.exists(path)){
...
uninstall_all_courses <- function(force = FALSE){
  ...
  if(file.exists(path)){
    if(!force){
      rproj_out(s()%N%"Are you sure you want to uninstall all rproj courses?",
                s()%N%"This will delete all of the contents of your rproj course directory.")
...
#' 
#' install_course_zip("~/Downloads/rproj_courses-master.zip", multi=TRUE,
...
install_course_zip <- function(path, multi=FALSE, which_course=NULL){
  ...
  if(multi){
    ...
    file_list <- unzip(path, exdir = rproj_courses_dir())
  }
...
install_course_directory <- function(path){
  # Check for size of directory to prevent copying a huge directory into rproj/Courses
  ...
  # Copy files
  if(file.copy(path, rproj_courses_dir(), recursive=TRUE)){
    rproj_out(s()%N%"Course installed successfully!", skip_after=TRUE)
  } else {
    rproj_out(s()%N%"Course installation failed.", skip_after=TRUE)
  }
...
install_course_url <- function(url, multi=FALSE){
  ...
  if(grepl("github.com", url) && !multi){
    ...
    file.rename(file.path(rproj_courses_dir(), old_name), 
                file.path(rproj_courses_dir(), course_name))
  }
