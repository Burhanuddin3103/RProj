# Instruction set for rproj.R's "virtual machine".
...
waitUser.video <- function(current.row, e){
  ...
  if(tolower(response) %in% c("y", "yes")){
    rproj_out(s()%N%"Type nxt() to continue")
    e$prompt <- TRUE
    e$playing <- TRUE
...
waitUser.script <- function(current.row, e){
  ...
  # Give instructions
  # rproj_out("INSTRUCTIONS: Edit the script and experiment in the console as much as you want. When you are ready to move on, SAVE YOUR SCRIPT and type submit() at the prompt. The script will remain open until you close it.",
...
# "between" function testMe and the rproj namespace. That is,
# an environment, customTests, is created with parent rproj
# and child testMe. Code evaluated within testMe will thus search
# for functions first in customTests, and then in the rproj namespace.
#
...
# An environment with parent rproj to hold custom tests.
