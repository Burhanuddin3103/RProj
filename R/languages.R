rproj_language <- function(){
  lang <- getOption("rproj_language")
...
#' Select a language
#' 
#' Select a language for rproj's menus.
#' @param language The language that rproj's menus will use.
...
#' \code{options(rproj_language = [selected language])} to the end of your 
...
select_language <- function(language = NULL, append_rprofile = FALSE){
  ...
  options(rproj_language = selection)
  
  if(append_rprofile){
    opts <- paste0("options(rproj_language = '", selection, "')")
    cat(opts, "\n", file = file.path("~", ".Rprofile"), append = TRUE)
  }
}

# set working directory to rproj repo before using
...
compile_languages <- function(){
  ...
  git_clone <- paste("git clone https://github.com/rprojdev/translations.git", clone_dir)
  system(git_clone)
  ...
  for(i in menus){
    ...
    cmd <- paste0(lang_name, " <- rproj:::wrap_encoding(yaml.load_file('", i, "'))")
    eval(parse(text=cmd))
  }
