  ...
  if(!exists("les",e,inherits = FALSE)){
    ...
    if(response != "" ){
      ...
      if(length(coursesU)==0){
        suggestions <- yaml.load_file(file.path(find.package("rproj"), "Courses", "suggested_courses.yaml"))
        choices <- sapply(suggestions, function(x)paste0(x$Course, ": ", x$Description))
        rproj_out(s()%N%"To begin, you must install a course. I can install a course for you from the internet, or I can send you to a web page (https://github.com/rprojdev/rproj_courses) which will provide course options and directions for installing courses yourself. (If you are not connected to the internet, type 0 to exit.)")
        ...
        if(n < length(choices)){
          repeat {
            ...
            if(is(temp, "try-error")){
              rproj_out(s()%N%"Sorry, but I'm unable to fetch ", sQuote(choice),
              ...
              if(resp == ch[2]) {
                rproj_out(s()%N%"OK. I'm opening the rproj course respository in your browser.")
                browseURL("https://github.com/Burhanuddin3103/RProj")
                return(FALSE)
              }
            } else {
      ...
      while(lesson == ""){
        ...
        if(!is.null(names(course)) && names(course)=="repo") {
          rproj_out(s()%N%"OK. I'm opening the rproj courses web page in your browser.")
          browseURL("https://github.com/Burhanuddin3103/RProj")
          return(FALSE)
        }
      ...
      # indicator that rproj is not reacting to console input
      e$playing <- FALSE
...
welcome.default <- function(e, ...){
  rproj_out(s()%N%"Welcome to rproj! Please sign in. If you've been here before, use the same name as you did then. If you are new, call yourself something unique.", skip_after=TRUE)
  resp <- readline(s()%N%"What shall I call you? ")
  while(str_detect(resp, '[[:punct:]]') || nchar(str_trim(resp)) < 1) {
    rproj_out(s()%N%"Please don't use any quotes or other punctuation in your name.",
              skip_after = TRUE)
    resp <- readline(s()%N%"What shall I call you? ")
  }
  return(resp)
...
housekeeping.default <- function(e){
  rproj_out(paste0(s()%N%"Thanks, ", e$usr, s()%N%". Let's cover a couple of quick housekeeping items before we begin our first lesson. First of all, you should know that when you see '...', that means you should press Enter when you are done reading and ready to continue."))
  readline(s()%N%"\n...  <-- That's your cue to press Enter to continue")
  rproj_out(s()%N%"Also, when you see 'ANSWER:', the R prompt (>), or when you are asked to select from a list, that means it's your turn to enter a response, then press Enter to continue.")
  select.list(c(s()%N%"Continue.", s()%N%"Proceed.", s()%N%"Let's get going!"),
              title=s()%N%"\nSelect 1, 2, or 3 and press Enter", graphics=FALSE)
  rproj_out(s()%N%"You can exit rproj and return to the R prompt (>) at any time by pressing the Esc key. If you are already at the prompt, type bye() to exit and save your progress. When you exit properly, you'll see a short message letting you know you've done so.")
  info()
  rproj_out(s()%N%"Let's get started!", skip_before=FALSE)
  readline("\n...")
}
...
inProgressMenu.default <- function(e, choices){
  ...
  rproj_out(s()%N%"Would you like to continue with one of these lessons?")
...
courseMenu.default <- function(e, choices){
  repo_option <- s()%N%"Take me to the rproj course repository!"
  choices <- c(choices, repo = repo_option)
  rproj_out(s()%N%"Please choose a course, or type 0 to exit rproj.")
...
lessonMenu.default <- function(e, choices){
  rproj_out(s()%N%"Please choose a lesson, or type 0 to return to course menu.")
  return(select.list(choices, graphics=FALSE))
}
...
restoreUserProgress.default <- function(e, selection){
  ...
  # transfer rproj's "official" list of variables to the
  # global environment.
...
getUser.default <- function(){"rprojadmin"}
