...
#' 
#' There are several ways to exit rproj: by typing \code{bye()} while in the R
#' console, by hitting the Esc key while not in the R console, or by
#' entering 0 from the rproj course menu. rproj will print a goodbye 
#' message whenever it exits. 
#' 
#' While rproj is in operation, it may be controlled by entering special
...
#' interference or commentary from rproj. This can be accomplished by
#' using the special command \code{play()}. rproj will remain in operation,
#' silently, until the special command \code{nxt()} is entered.
#' 
#' The special command \code{skip()} can be used to skip a question if 
#' necessary. rproj will enter the correct answer and notify the
...
#' rproj()
#' }
rproj <- function(resume.class="default", ...){
  ...
  # e lives here, in the environment created when rproj() is run
  e <- new.env(globalenv())
  ...
  # The callback also lives in the environment created when rproj()
...
#' Exit rproj.
#' 
#' rproj operates by installing a callback function which responds
#' to commands entered in the R console. This is how it captures
#' and tests answers given by the user in the R console. rproj will
...
#' > bye()
#' 
#' | Leaving rproj now. Type rproj() to resume.
#' }
bye <- function(){
  removeTaskCallback("mini")
  rproj_out(s()%N%"Leaving rproj now. Type rproj() to resume.", skip_after=TRUE)
  invisible()
}

#' Begin the upcoming question or unit of instruction.
#' 
#' This is the way to regain rproj's attention after viewing
...
#' 
#' rproj will enter the correct answer and notify the
#' user of the names of any new variables which it may have created
#' in doing so. These may be needed for subsequent questions.
#' @export
#' @examples
...
#' 
#' When a rproj question requires the user to edit an R script, the
...
#' Return to rproj's main menu.
#' 
#' Return to rproj's main menu from a lesson in progress.
#' @export
#' @examples
#' \dontrun{
#' 
...
#' Restart the current rproj lesson.
#'
#' Restart the current rproj lesson.
#'
#' @export
...
#' @export
info <- function(){
  rproj_out(s()%N%"When you are at the R prompt (>):")
  rproj_out(s()%N%"-- Typing skip() allows you to skip the current question.", skip_before=FALSE)
  rproj_out(s()%N%"-- Typing play() lets you experiment with R on your own; rproj will ignore what you do...", skip_before=FALSE)
  rproj_out(s()%N%"-- UNTIL you type nxt() which will regain rproj's attention.", skip_before=FALSE)
  rproj_out(s()%N%"-- Typing bye() causes rproj to exit. Your progress will be saved.", skip_before=FALSE)
  rproj_out(s()%N%"-- Typing main() returns you to rproj's main menu.", skip_before=FALSE)
  rproj_out(s()%N%"-- Typing info() displays these options again.", skip_before=FALSE, skip_after=TRUE)
  invisible()
}
...
resume.default <- function(e, ...){
  ...
  esc_flag <- TRUE
  on.exit(if(esc_flag)rproj_out(s()%N%"Leaving rproj now. Type rproj() to resume.", skip_after=TRUE))
  ...
  if(uses_func("skip")(e$expr)[[1]]){
    ...
    if(is(e$current.row, "script") && is.na(correctAns)) {
      ...
      if(file.exists(correct_script_path)) {
        ...
        rproj_out(s()%N%"I just sourced the following script, which demonstrates one possible solution.",
                  skip_after=TRUE)
    ...
    # If this is not a script question...
      ...
      rproj_out(s()%N%"Entering the following correct answer for you...",
                skip_after=TRUE)
  ...
  # if e$expr is NOT rproj() or nxt(), the user has just responded to
  ...
  if(!uses_func("rproj")(e$expr)[[1]] &&
       !uses_func("rprojify")(e$expr)[[1]] &&
  ...
  while(!e$prompt){
    ...
    if(e$row > min(nrow(e$les), e$test_to)) {
      ...
      # Save log
      if(isTRUE(getOption("rproj_logging"))){
        saveLog(e)
      }
      
      # Let user know lesson is complete
      rproj_out(s()%N%"You've reached the end of this lesson! Returning to the main menu...")
    ...
    if(e$iptr == 1){      
      ...
      #  of rproj's "official" names and values.
      if (!is.null(e$delta)){
      ...
      # Prepend the row's rproj class to its class attribute
