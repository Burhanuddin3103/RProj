rproj_out <- function(..., skip_before=TRUE, skip_after=FALSE) {
...
cleanAdmin <- function(){
  udat <- file.path(find.package("rproj"), "user_data", "rprojadmin")
...
loadDependencies <- function(lesson_dir) {
  ...
  if(file.exists(depends)) {
    ...
    rproj_out(s()%N%"Attempting to load lesson dependencies...")
    for(p in packages_as_chars) {
      ...
      if(suppressPackageStartupMessages(
        ...
        rproj_out(s()%N%"Package", sQuote(p), s()%N%"loaded correctly!")
      } else {
        rproj_out(s()%N%"This lesson requires the", sQuote(p), 
                  s()%N%"package. Would you like me to install it for you now?")
        yn <- select.list(choices=c(s()%N%"Yes", s()%N%"No"), graphics=FALSE)
        if(yn == s()%N%"Yes") {
          rproj_out(s()%N%"Trying to install package", sQuote(p), s()%N%"now...")
          ...
          if(suppressPackageStartupMessages(
            ...
            rproj_out(s()%N%"Could not install package", paste0(sQuote(p), "!"))
            return(FALSE)
          }
        } else {
          return(FALSE)
