  ...
  if(length(ls(envir=globalenv())) > 0) {
    packageStartupMessage(
      make_pretty(s()%N%"Hi! I see that you have some variables saved in your",
                  ...
                  s()%N%"before starting rproj.", skip_after=TRUE),
      ...
      make_pretty(s()%N%"Type rproj() when you are ready to begin.", skip_after=TRUE)
    )
  } else {
    packageStartupMessage(
      make_pretty(s()%N%"Hi! Type rproj() when you are ready to begin.",
                  skip_after=TRUE)
    )
  }
  invisible()
